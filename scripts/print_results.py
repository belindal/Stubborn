...
def evaluate(fn, print_prefix, alt_method_envids=set()):
    ...
    return alt_method_envids

alt_method_envids = evaluate("tolmroom_persisttop1_val_onlysamefloor/results.jsonl", "To LM Room (no fallback)")
...
evaluate("tolmroom_top1_val_onlysamefloor/results.jsonl", "To LM Room (total steps in room + proximity threshold)")
evaluate("togtroom_nofallback_val_onlysamefloor/results.jsonl", "To GT Room (no fallback)", alt_method_envids)
...
evaluate("torandroom_val_onlysamefloor/results.jsonl", "To rand room", alt_method_envids)
...
# for env_id in lm_fail_orig_succeed:
#     print(env_id, lm_fail_orig_succeed[env_id])
